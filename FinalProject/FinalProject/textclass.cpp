	...
	int screenWidth, int screenHeight, XMMATRIX baseViewMatrix)
{
	...
	result = InitializeSentence(&m_sentence8, 64, device);
	if (!result)
	{
		return false;
	}
...
bool TextClass::SetFPS(int fps, ID3D11DeviceContext* deviceContext)
{
	...
	result = UpdateSentence(m_sentence1, fpsString, 20, 20, 0.0f, 1.0f, 0.0f, deviceContext);
	if (!result)
	{
		return false;
	}
...
bool TextClass::SetScreenSize(int ScreenX, int ScreenY, ID3D11DeviceContext* deviceContext)
{
	...
	result = UpdateSentence(m_sentence3, ScreenXString, 20, 60, 1.f, 0.f, 0.f, deviceContext);
	...
	result = UpdateSentence(m_sentence4, ScreenYString, 20, 80, 1.f, 0.f, 0.f, deviceContext);
	if (!result) { return false; }

	return true;
}

bool TextClass::SetPolyCount(int polyCnt, ID3D11DeviceContext* deviceContext)
{
	char tempString[16];
	char countString[32];
	bool result;
	...
	// Update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_sentence5, countString, 20, 100, 0.0f, 0.0f, 1.0f, deviceContext);
	if (!result) { return false; }

	return true;
}

bool TextClass::SetObjCount(int ObjCnt, ID3D11DeviceContext* deviceContext)
{
	char tempString[16];
	char countString[16];
	bool result;
	...
	// Update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_sentence6, countString, 20, 120, 0.0f, 0.0f, 1.0f, deviceContext);
	if (!result) { return false; }
...
bool TextClass::SetHP(int hp, ID3D11DeviceContext* deviceContext)
{
	...
	// Setup the cpu string.
	strcpy_s(cpuString, " X ");
	strcat_s(cpuString, tempString);
	strcat_s(cpuString, "");

	// Update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_sentence7, cpuString, 760, 20, 1.0f, 0.0f, 0.0f, deviceContext);
	if (!result) { return false; }

	return true;
}

bool TextClass::SetTimer(int timer, ID3D11DeviceContext* deviceContext)
{
	char tempString[16];
	char cpuString[16];
	bool result;
	...
	// Convert the cpu integer to string format.
	_itoa_s(timer, tempString, 10);

	// Setup the cpu string.
	strcpy_s(cpuString, "Timer : ");
	strcat_s(cpuString, tempString);
	strcat_s(cpuString, "");

	// Update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_sentence8, cpuString, 730, 40, 0.0f, 1.0f, 0.0f, deviceContext);
	if (!result) { return false; }
